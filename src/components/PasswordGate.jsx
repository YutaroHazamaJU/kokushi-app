 ...
 *     return (
 *       <PasswordGate
 *         correctPassword="kokushi2025"          // 任意のパスワード
 *         storageKey="kokushi-app-unlocked"    // localStorage用キー（プロジェクトごとにユニークに）
 *       >
 *         <YourAppContents />  // ここに元々のアプリのJSXを入れる
 *       </PasswordGate>
 *     );
 *   }
 */
function PasswordGate({ children, correctPassword, storageKey = 'app-unlocked' }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!correctPassword) {
      setError('設定エラー：correctPassword が指定されていません');
      return;
    }
